    ...
    private mutating func visit(
        ...
        if cause.conflict.cause.isConflict && cause.other.cause.isConflict {
            ...
            if let conflictLine, let otherLine {
                try self.record(
                    incompatibility,
                    message: "\(incompatibilityDesc) because \(await self.description(for: cause.conflict)) (\(conflictLine)) and \(await self.description(for: cause.other)) (\(otherLine).",
                ...
                try self.record(
                    incompatibility,
                    message: "\(conjunction)\(incompatibilityDesc) because \(await self.description(for: withLine)) \(line).",
                    isNumbered: isNumbered
                )
            } else {
                ...
                if singleLineOther || singleLineConflict {
                    ...
                    try self.record(
                        incompatibility,
                        message: "\(conjunction)\(incompatibilityDesc) because \(await self.description(for: cause.conflict)) (\(self.lineNumbers[cause.conflict]!)).",
                        isNumbered: isNumbered
                    )
                }
            }
            ...
            if let derivedLine {
                try self.record(
                    incompatibility,
                    message: "\(incompatibilityDesc) because \(await self.description(for: ext)) and \(await self.description(for: derived)) (\(derivedLine)).",
                    isNumbered: isNumbered
                )
                ...
                try self.record(
                    incompatibility,
                    message: "\(conjunction)\(incompatibilityDesc) because \(await self.description(for: collapsedExt)) and \(await self.description(for: ext)).",
                    isNumbered: isNumbered
                )
            } else {
                ...
                try self.record(
                    incompatibility,
                    message: "\(conjunction)\(incompatibilityDesc) because \(await self.description(for: ext)).",
                    isNumbered: isNumbered
                )
            }
        } else {
            try self.record(
                incompatibility,
                message: "\(incompatibilityDesc) because \(await self.description(for: cause.conflict)) and \(await self.description(for: cause.other)).",
                isNumbered: isNumbered
            )
        }
    }
